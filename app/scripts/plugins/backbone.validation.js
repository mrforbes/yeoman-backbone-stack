...
Backbone.Validation = (function(Backbone, _, undefined) {
    ...
    var validateAttr = function(model, validation, attr, value, computed) {
        ...
        return _.reduce(validators, function(memo, validator){
            //michael forbes - added check for valid function... validation uses any non-predefined params as functions, but we want to add our
            //own custom properties for JNAP.
            ...
            if(validator.fn){
                ...
                if (result && !memo) {
                    //michael forbes - check to see if we are using a custom function - we always want to return the custom string from there.
                    if(_.isFunction(validator.val)){
                         return result || validator.msg;
                    }
                    else{
...
/** michael forbes - pass validation rules to callback **/
        for (changedAttr in attrs) {
            ...
            if (error) {
                ...
                isValid = false;
                if(view) options.invalid(view, changedAttr, error, options.selector, validation);
            } else {
                if(view) options.valid(view, changedAttr, options.selector, validation);
            }
    ...
    return {
        ...
        bind: function(view, options) {
            var model = view.model,
                ...
                /** michael forbes - add ability to pass in different model via options **/
...
/** MICHAEL FORBES - reset invalids */
            Backbone.Validation.invalids = {};
        ...
        unbind: function(view, options) {
            if(view){
                ...
                if(this.models){
                    ...
                    this.unbind('add', collectonAdd);
                    this.unbind('remove', collectionRemove);
...
/** MICHAEL FORBES - create an object to hold the invalids. clear it when binding. */
Backbone.Validation.invalids = {};
